$(document).ready(function() {
	...
	var explorer_tx = "https://explorer.superquantum.io/tx/"
	var explorer_addr = "https://explorer.superquantum.io/address/"
	var explorer_block = "https://explorer.superquantum.io/block/"
			...
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					var qrcode = new QRCode("walletQrCode", {
						colorDark: "hsl(165, 80%, 40%)",
						colorLight: "hsl(0, 0%, 8%)",
						correctLevel: QRCode.CorrectLevel.H
					});
					qrcode.makeCode("qubitcoin:"+address);
			...
			if(dvalue>=total){
				...
				tx2.broadcast(function(data){
					if(typeof data === 'string' && data.length === 64){
						// Success - data is the txid
						$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-success').html('txid: <a href="'+explorer_tx+data+'" target="_blank">'+data+'</a>');
					} else {
						// Error - display the error message
						var errorMsg = typeof data === 'string' ? data : (data.error || 'Unknown error');
						$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-danger').html(errorMsg);
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+dvalue+" QTC unable to send "+total+" QTC").fadeOut().fadeIn();
				thisbtn.attr('disabled',false);
			}
		...
		if($("#walletLoader").hasClass("hidden")){
			...
			coinjs.addressBalance($("#walletAddress").html(),function(data){
				if(data && data.chain_stats && data.mempool_stats){
					var v = ((data.chain_stats.funded_txo_sum || 0) + (data.mempool_stats.funded_txo_sum || 0) - (data.chain_stats.spent_txo_sum || 0) - (data.mempool_stats.spent_txo_sum || 0)) / 100000000;
					$("#walletBalance").html(v+" QTC").attr('rel',v).fadeOut().fadeIn();
				} else {
					$("#walletBalance").html("0.00 QTC").attr('rel',v).fadeOut().fadeIn();
				}
	...
	$("#transactionBtn").click(function(){
		...
		if(!$("#recipients .row, #inputs .row").hasClass('has-error')){
			...
			if($("#feesestnewtx").attr('est')=='y'){
				// fee estimation disabled; show notice instead of navigating
				alert('Fee estimation is a work in progress.');
			} else {

				$("#transactionCreate").removeClass("hidden");

				// Check fee against fluid 200 satoshis per byte calculation.
				if($("#transactionFee").val()>= estimatedTxSize * 200 * 1e-8){
	...
	// Disable direct fee estimation navigation; show WIP info instead
	$("#feesestnewtx, #feesestwallet").off('click').on('click', function(){
		alert('Fee estimation is a work in progress.');
	});
	...
	function listUnspentDefault(redeem){
		...
		if(redeem.from == 'txid'){
			tx.getTransaction(redeem.addr, function(data){
				...
				$.each(data.vin, function(i,o){
					var tx = o.txid;
					var n = o.vout;
					var amount = (o.prevout ? (o.prevout.value / 100000000).toFixed(8) : "0.00000000");

					var scr = o.prevout ? o.prevout.scriptpubkey : "";
				...
				$.each(data.vout, function(i,o){
					var address = o.scriptpubkey_address || "";
					var value = (o.value / 100000000).toFixed(8);
					addInput(address, value);
				});
	...
	function rawSubmitDefault(btn){ 
		...
		$.ajax ({
			...
			error: function(jqXHR, textStatus, errorThrown) {
				var errorMsg = jqXHR.responseText || errorThrown || "There was an error submitting your request";
				$("#rawTransactionStatus").addClass('alert-danger').removeClass('alert-success').removeClass("hidden").html(errorMsg).prepend('<span class="glyphicon glyphicon-exclamation-sign"></span> ');
			},
                        success: function(data) {
				if(typeof data === 'string' && data.length === 64){
					$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(' TXID: ' + data + '<br> <a href="' + explorer_tx + data + '" target="_blank">View on Explorer</a>');
				} else {
					$("#rawTransactionStatus").addClass('alert-danger').removeClass('alert-success').removeClass("hidden").html(' Unexpected response: ' + data).prepend('<span class="glyphicon glyphicon-exclamation-sign"></span> ');
				}
			},
			complete: function(data, status) {
	...
	$(".qrcodeBtn").click(function(){
		...
		if(ta.length>0){
			...
			qrstr = "qubitcoin:"+$('.address',thisbtn).val();
		}
